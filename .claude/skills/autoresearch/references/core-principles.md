...
## 8. Plateau Awareness and Continuity

Detect stalls early and respond with structured escalation. Persist strategic context across sessions.

| Stall Signal | Response |
|---|---|
| Improvement flattening (delta < threshold for N iterations) | EXPLOIT → REFINE → PIVOT (`references/plateau-detection.md`) |
| >5 consecutive discards | Novelty generation — analogical prompting, constraint inversion (`references/novelty-generation.md`) |
| Session boundary | Persist state to `autoresearch-state.md`, reload next session (`references/session-memory.md`) |

**Why:** Without plateau awareness, agents waste iterations repeating exhausted strategies. Without session memory, agents restart from zero every conversation — re-discovering dead ends and re-trying failed approaches. Together, these create a learning system that compounds across sessions.

**Apply:** Run plateau assessment every iteration (Phase 1). Follow EXPLOIT → REFINE → PIVOT escalation. Write session state at checkpoints (every 10 iterations, on PIVOT, at loop end). Load state at session start.
