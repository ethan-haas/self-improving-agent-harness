...
echo -e "iteration\tcommit\tmetric\tdelta\tguard\tstatus\tstrategy_category\tdescription" >> autoresearch-results.tsv
...
log_iteration() {
  local iteration=$1 commit=$2 metric=$3 delta=$4 guard=$5 status=$6 strategy=$7 description=$8
  echo -e "${iteration}\t${commit}\t${metric}\t${delta}\t${guard}\t${status}\t${strategy}\t${description}" \
    >> autoresearch-results.tsv
}

# Usage examples:
log_iteration 1 "b2c3d4e" "87.1" "+1.9" "pass" "keep" "exploit" "add tests for auth middleware"
log_iteration 2 "-" "86.5" "-0.6" "-" "discard" "refine" "refactor test helpers (broke 2 tests)"
log_iteration 3 "-" "0.0" "0.0" "-" "crash" "explore" "add integration tests (DB connection failed)"
...
```tsv
iteration	commit	metric	delta	guard	status	strategy_category	description
```
...
| strategy_category | enum | `exploit`, `explore`, `refine`, `pivot`, `combine`, `simplify`, `-` |
...
1	b2c3d4e	87.1	+1.9	pass	keep	exploit	add tests for auth middleware edge cases
2	-	86.5	-0.6	-	discard	refine	refactor test helpers (broke 2 tests)
3	-	0.0	0.0	-	crash	explore	add integration tests (DB connection failed)
4	-	88.9	+1.8	fail	discard	exploit	inline hot-path functions (guard: 3 tests broke)
5	c3d4e5f	88.3	+1.2	pass	keep	explore	add tests for error handling in API routes
6	d4e5f6g	89.0	+0.7	pass	keep	refine	add boundary value tests for validators
```
