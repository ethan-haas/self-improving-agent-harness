...
When bounded, track `current_iteration` against `max_iterations` using these exact semantics:

- **Baseline = iteration 0.** The initial setup measurement is iteration 0 and does NOT count toward N.
- **Experiments = iterations 1..N.** The first experiment is iteration 1. After completing iteration N (the Nth experiment), print the final summary and STOP.
- **Increment timing:** Increment `current_iteration` at the END of each iteration, AFTER logging (Phase 7). Check the termination condition at the START of Phase 8 (Repeat).
- **Example:** `--iterations 1` means: baseline (iter 0) + exactly 1 experiment (iter 1) + final summary + STOP.
...
### Sentinel File for Stop Hook (Unbounded Mode Only)

If the loop mode is **unbounded** (no `Iterations: N`), create the sentinel file `.autoresearch-active` so the stop hook can detect an active loop and block premature stops:

```bash
# Only for unbounded mode — bounded loops should stop at N
if [ -z "$MAX_ITERATIONS" ]; then
  cat > .autoresearch-active << EOF
Goal: ${GOAL}
Scope: ${SCOPE}
Verify: ${VERIFY_CMD}
Guard: ${GUARD_CMD}
Direction: ${DIRECTION}
EOF
  # Ensure sentinel is gitignored
  grep -qxF '.autoresearch-active' .gitignore 2>/dev/null || echo '.autoresearch-active' >> .gitignore
fi
```

**Do NOT create this file for bounded mode.** Bounded loops should stop naturally after N iterations.

The stop hook (`scripts/autoresearch-stop-hook.sh`) checks for this file — if it exists, the hook blocks Claude from stopping and feeds back the loop config as a continuation prompt.
...
7. If autoresearch-state.md exists, load session memory (see references/session-memory.md)
8. Run plateau assessment on last 10 results entries (see references/plateau-detection.md)
```
...
**Plateau-aware strategy selection:** Before choosing, check plateau status from Phase 1. Follow EXPLOIT → REFINE → PIVOT framework (`references/plateau-detection.md`). Use diversity enforcement (`references/novelty-generation.md`) when generating hypotheses.
...
```
iteration  commit   metric   status        strategy_category  description
42         a1b2c3d  0.9821   keep          [exploit]          increase attention heads from 8 to 12
43         -        0.9845   discard       [refine]           switch optimizer to SGD
44         -        0.0000   crash         [pivot]            double batch size (OOM)
...
### Unbounded Mode — Structural Loop Continuation

In unbounded mode, the loop runs until the user presses Ctrl+C. No other termination condition exists.

#### Structural Rule: Every Response MUST End With a Tool Call

The loop continuation problem is structural, not motivational. Text-based "never stop" instructions have been tried and failed repeatedly. Instead, use this mechanical rule:

**After Phase 7 (Log), your response MUST end with a tool call that begins Phase 1 of the next iteration.** Specifically: end every iteration by calling Bash to run `tail -5 autoresearch-results.tsv` or by calling Read on an in-scope file. This starts Phase 1 and prevents your response from ending with text — which is when the stop-bug triggers.

```
WRONG (ends with text → Claude stops):
    ... log result ...
    "52 experiments, 2 keeps. The loop continues."
    [END OF RESPONSE — Claude stops here]

RIGHT (ends with tool call → Claude continues):
    ... log result ...
    [Bash: tail -5 autoresearch-results.tsv]
    → tool result arrives → Claude processes it → Phase 1 continues → next iteration begins
```

**Why this works:** Claude stops when it generates text that "feels like" a conclusion. If the last thing in every response is a tool call, there is no concluding text, so the stop impulse never triggers. The tool result then feeds back into the conversation and the loop naturally continues.

> **BOUNDED MODE EXCEPTION:** The "must end with a tool call" rule applies ONLY to unbounded mode. In bounded mode, when `current_iteration >= max_iterations`, your response MUST end with the final summary text and NO tool call. Bounded termination has ABSOLUTE PRIORITY over the tool-call continuation rule. Making a tool call after the final summary would accidentally restart the loop.

#### When You Have 5+ Consecutive Discards

Long discard streaks (10, 20, 39 consecutive) are when the stop-bug is most likely to trigger. The agent feels like it's failing and wants to "report back." Instead:

1. **Write state checkpoint** to `autoresearch-state.md` (preserves learnings for auto-compact recovery)
2. **Enter PIVOT mode** — re-read ALL files, use adversarial negative mining, cross-domain analogical prompting
3. **Immediately begin the next iteration** with a tool call (read file or run git log)

Do NOT write a summary of findings. Do NOT report the discard count. Just pivot and continue.

#### Context Hygiene

Claude Code auto-compacts when context fills (~1M tokens). To maximize iterations:
- Use `git diff HEAD~1` instead of re-reading entire files
- Pipe verbose verify output through `tail -3`
- Read files with the Read tool silently — don't echo contents in text
- Keep commit messages to one sentence
- Use `tail -10` on results log, not `cat`

### Bounded Mode (with Iterations: N)

```
# At the END of each iteration, after Phase 7 (Log):
current_iteration += 1   # increment AFTER logging

# Phase 8 check:
IF current_iteration < max_iterations:
    # More iterations remain — continue the loop
    Go to Phase 1 (start next iteration with a tool call)
ELIF current_iteration >= max_iterations OR goal_achieved:
    rm -f .autoresearch-active   # Clean up sentinel (safety — shouldn't exist in bounded mode)
    IF goal_achieved:
        Print: "Goal achieved at iteration {current_iteration}! Final metric: {value}"
    Print final summary
    STOP                         # End response with TEXT, not a tool call.
                                 # The tool-call continuation rule does NOT apply here.
```
...
#### Bounded Mode: Edge Case Counting Rules

These rules define what increments `current_iteration` and what does not:

| Event | Increments `current_iteration`? | Rationale |
|-------|-------------------------------|-----------|
| Successful experiment (keep) | YES | Normal iteration completed |
| Failed experiment (discard) | YES | Iteration completed, experiment reverted |
| Crash (after fix attempts) | YES | Iteration consumed even though it failed |
| no-op (no diff produced) | YES | The iteration slot was used; pick a different approach next time |
| hook-blocked (after fix attempts) | YES | The iteration slot was used |
| Guard rework attempts (max 2) | NO | Rework attempts are part of the SAME iteration, not separate ones |
| Crash fix attempts (max 3) | NO | Fix attempts are part of the SAME iteration |
| Syntax error fix | NO | Immediate fix within the same iteration |

**Context compaction recovery:** If auto-compact occurs mid-bounded-loop, the agent may lose track of `current_iteration`. Recovery: count the non-baseline entries in `autoresearch-results.tsv` (lines where iteration > 0). That count IS `current_iteration`.

### When Stuck (>5 consecutive discards, unbounded mode only)
...
7. Consult `references/plateau-detection.md` for EXPLOIT → REFINE → PIVOT escalation
8. Consult `references/novelty-generation.md` for creativity techniques
9. If session memory exists, review cross-session history for untried approaches
...
- **EVERY response in unbounded mode MUST end with a tool call** — this is what keeps the loop alive. Text endings kill the loop.
- **DO NOT** ask "should I keep going?" — in unbounded mode, YES. ALWAYS.
- **DO NOT** summarize findings between iterations — just log to TSV and continue
- **DO NOT** count or report consecutive discards — this triggers the stop impulse. The results log tracks this; you don't need to narrate it.
- **DO** print a one-line status every ~10 iterations (e.g., "iter 25: 0.95, 8 keeps") — keep it SHORT, then immediately make a tool call to start the next iteration
- **DO** print a final summary when bounded loop completes (bounded ONLY)
- **In bounded mode, the FINAL response MUST end with text (the summary), NOT a tool call.** The tool-call rule is for unbounded continuation only. Ending with a tool call after the final bounded iteration would restart the loop.
