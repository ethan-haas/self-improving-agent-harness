...
Extract these from user arguments — the user may provide extensive context alongside config. Ignore prose and extract ONLY structured fields:
...
If `Iterations: N` or `--iterations N` is found, set `max_iterations = N`. Baseline measurement is iteration 0 (does NOT count toward N). Experiments are iterations 1 through N. Increment `current_iteration` after each iteration's log phase. After the Nth experiment completes, print final summary and STOP. Example: `--iterations 1` = baseline + exactly 1 experiment + summary + stop.
...
- User says "learn this codebase", "generate docs", "document this project", "create documentation", "update docs", "check docs health", "docs status" -> run the learn workflow
...
LOOP (FOREVER or N times):
  1. Review: Read current state + git history + results log + session memory + plateau check
  ...
  9. Repeat:
     - If unbounded: ALWAYS end response with a tool call (e.g. read results
       log or in-scope file). NEVER end with text — text endings kill the loop.
     - If bounded AND current_iteration >= max_iterations: Print final summary
       and STOP. End with TEXT, not a tool call. Bounded termination has
       ABSOLUTE PRIORITY over the tool-call rule.
     - If bounded AND current_iteration < max_iterations: End with a tool call
       to start the next iteration (same as unbounded behavior).
```
...
9. **Stop hook sentinel** — In unbounded mode, create `.autoresearch-active` at Phase 0 with loop config. This enables the stop hook to block premature stops and feed back continuation context. Do NOT create for bounded mode. Clean up on completion
10. **Bounded termination is absolute** — When `current_iteration >= max_iterations`, STOP. Print the final summary and end with text. Do NOT make a tool call. The "end with tool call" rule applies only to unbounded mode and to non-final bounded iterations. Guard rework and crash fix attempts do not increment the iteration counter; they are part of the current iteration.
...
|----------|-----------|---------|
| Plateau Detection | `references/plateau-detection.md` | Stall detection, EXPLOIT -> REFINE -> PIVOT escalation |
| Session Memory | `references/session-memory.md` | Cross-session persistence via `autoresearch-state.md` |
| Novelty Generation | `references/novelty-generation.md` | Diversity enforcement, creative prompting techniques |
...
| Documentation | Learn score (higher) | Codebase + docs/ | `/autoresearch:learn` | — |
